...
class MediaTestPage < BaseTestPage
  ...
  audios  audio_player_1: 'audio#audio_player1',
          audio_player_2: 'audio#audio_player2'
  label   :audio_caption, 'div#vtt-text'
  ...
  def verify_page_ui
    ...
    audio_player_1.wait_until_ready_state_is(4, 10)
    ui = {
      video_player_1 => {
        visible: true,
        paused: true,
      ...
      audio_player_2 => {
        ...
        active_track_source: { ends_with: 'subtitles/bbc-pm.vtt' },
        all_tracks_data: [
          { 1 => { kind: 'subtitles', label: 'English', language: 'en', mode: 'showing' } }
        ],
      },
      audio_caption => { exists: false }
    }
    verify_ui_states(ui)
         ...
         when audio_player_2
           {
             player => {
               ...
               active_track_cue_count: 97,
               active_cue_text: ' A lesson from the Spanish-Portuguese power cut yesterday.',
               active_cue_data: {
                 text: ' A lesson from the Spanish-Portuguese power cut yesterday.',
                 start: 0,
                 end: 2.6
               }
             }
           ...
           raise 'Specified player does not support closed captions'
         end
    verify_ui_states(ui)
  end
         ...
         when audio_player_2
           {
             player => {
               ...
               ready_state: 4,
               current_time: 20,
               active_cue_text: ' even when the electricity grid has collapsed.',
               active_cue_data: {
                 text: ' even when the electricity grid has collapsed.',
                 start: 18.22,
                 end: 20.8
               }
             },
             audio_caption => {
               visible: true,
               caption: ' even when the electricity grid has collapsed.'
             }
           }
         else
           raise 'Specified player does not support closed captions'
